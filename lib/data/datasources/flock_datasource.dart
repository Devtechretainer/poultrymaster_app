...
class FlockDataSource {
  ...
  FlockDataSource({required this.dio, required this.baseUrl});
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data is List) {
          return data
              .map(
                (json) => FlockModel.fromJson(json as Map<String, dynamic>),
              )
              .toList();
        }
        return [];
  ...
  Future<FlockModel?> getFlockById(
    int flockId,
    String userId,
    String farmId,
    String? token,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to load flock: ${response.statusCode}');
      }
    } catch (e) {
      if (e is DioException) {
        throw Exception('API Error: ${e.message}');
    ...
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        final errorMessage =
            ...
            'Failed to create flock: ${response.statusCode}';
